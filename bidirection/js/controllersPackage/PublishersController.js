  ...
  function ( $scope, $location, $http) {
    ...
    $scope.getAllPublishers = function () {
      $http( {
        ...
        .success( function ( data) {
          ...
          var counter = $scope.publishers.length;
          $scope.publishers.forEach( function ( p) {
            $http( {
              ...
              .error( function ( data) { console.log("Error for get corresponding books: ", data);});
          });
        })
        .error( function ( data) {
          console.log( data);
    ...
    $scope.selectPublisher = function () {
      ...
      $scope.$watchCollection( 'publisher', function ( newPublisher) {
        if ( angular.equals( newPublisher, tempPublisher)) {
          $scope.publisherInfo.$setPristine();
        }
      });
    };
     ...
     * - If the request is succeed, remove publisher property from corresponding book record then
     *   jump to parent page;
     * - otherwise, show error messages in console.
     */
    $scope.destroyPublisher = function () {
      $http( {
        ...
        .success( function () {
          // Begin: delete the pointer from book
          $http( {
            method: 'GET',
            url: 'https://api.parse.com/1/classes/Book',
            params: {
              where: {
                publisher: {
                  __type: 'Pointer',
                  className: 'Publisher',
                  objectId: $scope.publisher.objectId
                }
              }
            }
          })
            ...
            .error( function ( data) { console.log( data);});
          // End: delete the pointer from book
          $location.path('/publishers/managePublishers');
        })
        .error( function ( data) {
          console.log( data);
